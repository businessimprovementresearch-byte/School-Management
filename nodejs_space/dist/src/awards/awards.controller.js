"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.AwardsController = void 0;
const openapi = require("@nestjs/swagger");
const common_1 = require("@nestjs/common");
const swagger_1 = require("@nestjs/swagger");
const awards_service_1 = require("./awards.service");
const jwt_auth_guard_1 = require("../auth/guards/jwt-auth.guard");
const roles_guard_1 = require("../auth/guards/roles.guard");
const roles_decorator_1 = require("../auth/decorators/roles.decorator");
const award_dto_1 = require("./dto/award.dto");
let AwardsController = class AwardsController {
    service;
    constructor(service) {
        this.service = service;
    }
    async findAll() {
        return this.service.findAll();
    }
    async findIssuances(studentId, teacherId) {
        return this.service.findIssuances(studentId, teacherId);
    }
    async create(dto) {
        return this.service.create(dto);
    }
    async remove(id) {
        return this.service.remove(id);
    }
    async update(id, dto) {
        return this.service.update(id, dto);
    }
    async issue(dto) {
        return this.service.issue(dto);
    }
    async removeIssuance(id) {
        return this.service.removeIssuance(id);
    }
};
exports.AwardsController = AwardsController;
__decorate([
    (0, common_1.Get)(),
    openapi.ApiResponse({ status: 200, type: [require("./dto/award.dto").AwardListItemDto] }),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", Promise)
], AwardsController.prototype, "findAll", null);
__decorate([
    openapi.ApiQuery({ name: "studentId", required: false }),
    openapi.ApiQuery({ name: "teacherId", required: false }),
    (0, common_1.Get)('issuances'),
    openapi.ApiResponse({ status: 200, type: [require("./dto/award.dto").AwardIssuanceDto] }),
    __param(0, (0, common_1.Query)('studentId')),
    __param(1, (0, common_1.Query)('teacherId')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, String]),
    __metadata("design:returntype", Promise)
], AwardsController.prototype, "findIssuances", null);
__decorate([
    (0, common_1.Post)(),
    (0, roles_decorator_1.Roles)('ADMIN'),
    openapi.ApiResponse({ status: 201, type: require("./dto/award.dto").AwardListItemDto }),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [award_dto_1.CreateAwardDto]),
    __metadata("design:returntype", Promise)
], AwardsController.prototype, "create", null);
__decorate([
    (0, common_1.Delete)(':id'),
    (0, roles_decorator_1.Roles)('ADMIN'),
    openapi.ApiResponse({ status: 200, type: require("../common/dto/success-response.dto").SuccessResponseDto }),
    __param(0, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", Promise)
], AwardsController.prototype, "remove", null);
__decorate([
    (0, common_1.Patch)(':id'),
    (0, roles_decorator_1.Roles)('ADMIN'),
    openapi.ApiResponse({ status: 200, type: require("./dto/award.dto").AwardListItemDto }),
    __param(0, (0, common_1.Param)('id')),
    __param(1, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, award_dto_1.UpdateAwardDto]),
    __metadata("design:returntype", Promise)
], AwardsController.prototype, "update", null);
__decorate([
    (0, common_1.Post)('issue'),
    (0, roles_decorator_1.Roles)('ADMIN'),
    openapi.ApiResponse({ status: 201, type: require("../common/dto/success-response.dto").SuccessResponseDto }),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [award_dto_1.IssueAwardDto]),
    __metadata("design:returntype", Promise)
], AwardsController.prototype, "issue", null);
__decorate([
    (0, common_1.Delete)('issuances/:id'),
    (0, roles_decorator_1.Roles)('ADMIN'),
    openapi.ApiResponse({ status: 200, type: require("../common/dto/success-response.dto").SuccessResponseDto }),
    __param(0, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", Promise)
], AwardsController.prototype, "removeIssuance", null);
exports.AwardsController = AwardsController = __decorate([
    (0, swagger_1.ApiTags)('Awards'),
    (0, common_1.Controller)('api/awards'),
    (0, common_1.UseGuards)(jwt_auth_guard_1.JwtAuthGuard, roles_guard_1.RolesGuard),
    __metadata("design:paramtypes", [awards_service_1.AwardsService])
], AwardsController);
//# sourceMappingURL=awards.controller.js.map